...
import com.proyecto.reservaCanchas.exception.UnauthorizedActionException;
...
public class CanchaServiceImpl implements CanchaService {
    ...
    public CanchaResponseDTO createCancha(CanchaRequestDTO canchaRequestDTO) {
        ...
        User admin = userRepository.findById(canchaRequestDTO.getAdminId())
                .orElseThrow(() -> new ResourceNotFoundException("El Usuario con el id " + canchaRequestDTO.getAdminId() + " no existe"));

        // Validar que el usuario tiene rol de "admin"
        if (!admin.getRol().getNombre().equals("admin")) {
            throw new UnauthorizedActionException("El usuario no tiene el rol de admin, no puede crear canchas");
        }
